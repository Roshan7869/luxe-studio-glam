...
bloginfo('name'); ?></title>
<meta name="description" content="GlamLux2Lux — India's premier luxury beauty franchise. Book appointments, explore services, and own a franchise location today.">
<?php wp_head(); ?>

<!-- Google Fonts -->
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,600;0,700;1,400&family=Cormorant+Garamond:ital,wght@0,400;0,600;1,400&family=Inter:wght@300;400;500;600&display=swap" rel="stylesheet"/>
...
:root{
  ...
  --dark:#0F0F0F;--dark-2:#1A1A1A;--dark-3:#242424;
  --off-white:#F8F7F3;--off-white-2:#EEECEA;
  --text-primary:#0F0F0F;--text-secondary:#666;--text-muted:#999;
...
h1,h2,h3,h4{font-family:'Playfair Display',Georgia,serif;line-height:1.15;letter-spacing:-0.02em;}
...
/* Logo */
.gl-logo{display:flex;align-items:center;gap:11px;text-decoration:none;}
.gl-logo-mark{
...
.gl-logo-mark span{font-family:'Playfair Display',serif;font-weight:700;color:#fff;font-size:0.9375rem;line-height:1;}
.gl-logo-text{font-family:'Playfair Display',serif;font-weight:700;font-size:1.0625rem;color:var(--dark);letter-spacing:-0.01em;}
.gl-logo-text em{color:var(--gold);font-style:normal;}

/* Nav links */
.gl-nav-links{display:flex;align-items:center;gap:4px;list-style:none;}
.gl-nav-links a{
  position:relative;padding:6px 14px;
  font-size:0.8375rem;font-weight:500;color:var(--text-secondary);
  letter-spacing:0.01em;border-radius:9999px;
  transition:color var(--transition-fast),background var(--transition-fast);
}
.gl-nav-links a:hover{color:var(--dark);background:rgba(0,0,0,0.04);}
.gl-nav-links a.active{color:var(--dark);font-weight:600;}
...
.gl-nav-cta{
  ...
  font-size:0.8125rem;font-weight:600;letter-spacing:0.035em;
  box-shadow:0 4px 16px rgba(198,167,94,0.30);
  transition:transform var(--transition-fast),box-shadow var(--transition-fast),background var(--transition-fast);
}
.gl-nav-cta:hover{background:var(--gold-dark);transform:translateY(-1px);box-shadow:0 8px 24px rgba(198,167,94,0.40);}
.gl-nav-cta svg{transition:transform var(--transition-fast);}
.gl-nav-cta:hover svg{transform:translateX(3px);}

/* Mobile toggle */
.gl-nav-toggle{display:none;background:none;border:none;padding:6px;border-radius:var(--radius-sm);flex-direction:column;gap:5px;cursor:pointer;}
.gl-nav-toggle span{display:block;width:22px;height:1.5px;background:var(--dark);border-radius:2px;transition:all var(--transition-base);}

/* Toast container */
#gl-toast-container{position:fixed;bottom:28px;right:28px;z-index:9000;display:flex;flex-direction:column;gap:10px;}
...
@media(max-width:860px){
  ...
  .gl-nav-links.open{
    ...
    padding:20px 24px 28px;border-bottom:1px solid rgba(0,0,0,0.06);
    box-shadow:0 16px 40px rgba(0,0,0,0.08);
  }
  .gl-nav-links.open a{padding:10px 14px;width:100%;}
...
<nav id="site-nav" role="navigation" aria-label="Main navigation">
  ...
  <div style="display:flex;align-items:center;gap:12px;">
    <a href="<?php echo esc_url(home_url('/franchise-enquiry')); ?>" class="gl-nav-cta" id="nav-cta">
      Join the Franchise
      <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M2.5 6.5h8M6.5 2.5l4 4-4 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
    </a>
