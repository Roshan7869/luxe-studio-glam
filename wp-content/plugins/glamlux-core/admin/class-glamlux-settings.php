...
?>
	...
	public function page_init()
	{
		...
		register_setting(
			'glamlux_frontend_group',
			'glamlux_razorpay_key_id',
			'sanitize_text_field'
		);

		add_settings_section(
			'glamlux_payment_section',
		...
		add_settings_field(
			'glamlux_razorpay_key_id',
			'Razorpay Key ID',
			array($this, 'razorpay_key_callback'),
			'glamlux-frontend-settings',
			'glamlux_payment_section'
		);
	}
	...
	{
		print 'Configure keys for processing global client transactions (Used for Payroll, Memberships & Retail):';
	}
	...
	{
		printf(
			'<input type="text" id="glamlux_razorpay_key_id" name="glamlux_razorpay_key_id" value="%s" class="regular-text" placeholder="rzp_test_..." />',
			esc_attr(get_option('glamlux_razorpay_key_id', ''))
		);
	}
}
